...
@Component({
  selector: 'app-contrato-transporte-nacional',
  templateUrl: './contrato-transporte-nacional.component.html',
  styleUrls: ['./contrato-transporte-nacional.component.css'],
  providers: [
    {provide: NgbDateParserFormatter, useClass: NgbDateCustomParserFormatter},
    { provide: NgbDatepickerI18n, useClass: NgbDatepickerCustomI18n }
   ]  
})
export class ContratoTransporteNacionalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getLotesTransporteNacional()
    
    
    if (+this.idContract > 0){
      this.isNew = false;
      this.contratosService.obtenerContratoNacional(+this.idContract)
        .subscribe(
          (result: any) => {
            var datum = result; 
            if (datum.success) {
  ...
  getSectores(){
    this.parametricasService.obtenerParametrica('SECTORES_NACIONALES')
      .subscribe(
        result => {
  ...
  getProductos(){
    this.parametricasService.obtenerParametrica('PRODUCTOS_NACIONALES')
      .subscribe( 
        result => {
    ...
    .subscribe(
      result => {
        ...
        if (lotesTransInter.success) {
          ...
          this.lotesContexto = this.lotesTransInter.map((element:any)=> ({id: element.id, checked:false, lote:element.lote}))
        }
      },
    error => {
      console.log('Retrieving problems!');
    ...
    .subscribe(
      result => {
        if (result.success) {
          ...
          dataLote.pOrigenes = dataLote.pOrigenes.sort((a:any,b:any)=>{if (a.descripcion<b.descripcion) return -1; return 1;})
          ...
          dataLote.pDestinos = dataLote.pDestinos.sort((a:any,b:any)=>{if (a.descripcion<b.descripcion) return -1; return 1;})
    ...
    this.lotesContexto.filter((element:any)=>element.checked)
      .forEach((elementLoteContexto:any)=>{
        ...
        if (!existe){
          ...
          // this.puntoRecepciones = dataLoteSeleccionado.pOrigenes;
          // this.fronteras = dataLoteSeleccionado.pFronteras;
          // this.puntoEntregas = dataLoteSeleccionado.pDestinos;
  ...
  save(){
    var result:any;
    this.contratosService.guardarContratoNacional(this.data).subscribe({
      next: (datum: any[]) => {
        console.log(datum);
        result = datum;
        if (result.success){
  ...
  update(){
    ...
    this.contratosService.modificarContratoNacional(this.idContract, this.data, this.justificacionCambio).subscribe({
      next: (datum: any[]) => {
        console.log(datum);
        result = datum;
        if (result.success){
