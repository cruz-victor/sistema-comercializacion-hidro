...
@Component({
  selector: 'app-contrato-transporte',
  templateUrl: './contrato-transporte.component.html',
  styleUrls: ['./contrato-transporte.component.css'],
  providers: [
    {provide: NgbDateParserFormatter, useClass: NgbDateCustomParserFormatter},
    { provide: NgbDatepickerI18n, useClass: NgbDatepickerCustomI18n }
   ]  
})
export class ContratoTransporteComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (+this.idContract > 0){
      this.isNew = false;
      this.contratosService.obtenerContrato(+this.idContract)
        .subscribe(
          (result: any) => {
            var datum = result; 
            if (datum.success) {
  ...
  save(){
    var result:any;
    this.contratosService.guardarContrato(this.data).subscribe({
      next: (datum: any[]) => {
        console.log(datum);
        result = datum;
        if (result.success){
  ...
  update(){
    ...
    this.contratosService.modificarContrato(this.idContract, this.data, this.justificacionCambio).subscribe({
      next: (datum: any[]) => {
        console.log(datum);
        result = datum;
        if (result.success){
