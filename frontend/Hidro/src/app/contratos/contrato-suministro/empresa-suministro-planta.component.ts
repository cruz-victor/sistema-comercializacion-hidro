...
@Component({
  selector: 'app-empresa-suministro-planta',
  templateUrl: './empresa-suministro-planta.component.html',
  styleUrls: ['./empresa-suministro-planta.component.css']
})
export class EmpresaSuministroPlantaComponent implements OnInit {
  ...
  initEmpresa(){
    this.empresa={
      ...
      tipoConstitucion: "PLANTA_BUQUE",            email: '',     direccion: '',            sitioWeb: '',              telefono: '',
  ...
  validate(): boolean{
    ...
    if (this.empresa.razonSocial.trim()===''){
      this.showToast('dark', 'El nombre de la empresa debe ser válido.')
       ...
       result => {
        this.empresas = result; 
         if (result.success) {
          ...
          this.empresas =  this.empresas.filter((element:any)=>element.tipoConstitucion=="PLANTA_BUQUE")
            // .sort((a:any, b:any)=>{if (a.razonSocial<b.razonSocial) return -1; else return 1})
         }
     },
     error => {
